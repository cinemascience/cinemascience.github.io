...
window.CINEMA_COMPONENTS=CINEMA_COMPONENTS;CINEMA_COMPONENTS.DATABASE_INCLUDED=true;CINEMA_COMPONENTS.DIMENSION_TYPE=Object.freeze({INTEGER:0,FLOAT:1,STRING:2});CINEMA_COMPONENTS.Database=function(directory,callback,errorCallback,filter){this.directory=directory;this.loaded=false;this.error;this.data=[];this.dimensions=[];this.dimensionTypes={};this.dimensionDomains={};this.filter=filter
this.hasAxisOrdering=false;this.axisOrderData;this.dispatch=d3.dispatch("dataUpdated");this.errorCallback=errorCallback;var self=this;self.path=directory+'/data.csv';getAndParseCSV(self.path,function(data_arr,request){self.prevContentLength=request.getResponseHeader('Content-Length');self.error=checkErrors(data_arr);if(self.error){console.warn(self.error);if(errorCallback)
errorCallback();return;}
calcData(self,data_arr);getAndParseCSV(directory+'/axis_order.csv',function(axis_data_arr){var error=checkAxisDataErrors(axis_data_arr,self.dimensions);if(!error){self.hasAxisOrdering=true;self.axisOrderData=parseAxisOrderData(axis_data_arr);}
else
...
errorCallback("Error loading data.csv!");});};CINEMA_COMPONENTS.Database.prototype.isStringDimension=function(dimension){return this.dimensionTypes[dimension]===CINEMA_COMPONENTS.DIMENSION_TYPE.STRING;};var calcData=function(self,data_arr){self.dimensions=data_arr[0];self.data=data_arr.slice(1).map(function(d){var obj={};self.dimensions.forEach(function(p,i){obj[p]=d[i];});return obj;});var filterdOut=[]
self.dimensions.forEach(function(d){var val=self.data[0][d];var i=0;while(val===undefined&&i<self.data.length)
val=self.data[++i][d];if(!isNaN(val)||val.toUpperCase()==="NAN"){if(isNaN(val)||!Number.isInteger(val))
self.dimensionTypes[d]=CINEMA_COMPONENTS.DIMENSION_TYPE.FLOAT;else
self.dimensionTypes[d]=CINEMA_COMPONENTS.DIMENSION_TYPE.INTEGER;var filter=self.filter?self.filter[d]:null
if(filter&&(!Array.isArray(filter)||filter.length!=2)){console.warn("Filter for dimension '"+d+"' must be an array of length two.")
filter=null}
var i;for(i=0;i<self.data.length&&isNaN(self.data[i][d]);i++){}
if(i==self.data.length)
self.dimensionDomains[d]=[0,0]
else{var min=self.data[i][d];var max=self.data[i][d];if(filter){min=Math.max(min,filter[0])
max=Math.min(max,filter[1])}
for(var j=i;j<self.data.length;j++){if(!isNaN(self.data[j][d])){if(filter&&(self.data[j][d]<filter[0]||self.data[j][d]>filter[1])){filterdOut[j]=true;continue;}
min=Math.min(min,self.data[j][d]);max=Math.max(max,self.data[j][d]);}}
self.dimensionDomains[d]=[min,max];}}
else{self.dimensionTypes[d]=CINEMA_COMPONENTS.DIMENSION_TYPE.STRING;self.dimensionDomains[d]=self.data.map(function(p){return p[d];});}});self.data=self.data.filter(function(d,i){return!filterdOut[i]});};CINEMA_COMPONENTS.Database.prototype.refreshData=function(reloadAllData){var self=this;if(reloadAllData){getAndParseCSV(self.path,function(data_arr,request){dataUpdateCallback(self,data_arr,request);},self.errorCallback);}
else{var xhReq=new XMLHttpRequest();xhReq.open("HEAD",self.path,true);xhReq.onreadystatechange=function(){if(xhReq.readyState===4){if(xhReq.status===200||(navigator.userAgent.match(/Safari/)&&xhReq.status===0)){var contentLength=xhReq.getResponseHeader('Content-Length');if(contentLength!=self.prevContentLength){getAndParseCSV(self.path,function(data_arr,request){dataUpdateCallback(self,data_arr,request);},self.errorCallback);}}}}
xhReq.send(null);}}
var dataUpdateCallback=function(self,data_arr,request){if(data_arr[0].length!=self.dimensions.length){console.warn("Updates to data cannot change the number of dimensions!")
return;}
for(var i in self.dimensions){if(self.dimensions[i]!=data_arr[0][i]){console.warn("Updates to data cannot change the names of dimensions!")
return;}}
var error=checkErrors(data_arr);if(error){console.warn("Error in updated data!\n"+error);return;}
self.prevContentLength=request.getResponseHeader('Content-Length');var newData=data_arr.slice(1).map(function(d){var obj={};self.dimensions.forEach(function(p,i){obj[p]=d[i];});return obj;});var updated=false;var updateInfo={added:[],modified:[],removed:[],oldData:self.data,oldDimensionDomains:self.dimensionDomains};for(var f=0;f<self.data.length||f<newData.length;f++){if(f>=self.data.length){updateInfo.added.push(f);updated=true;}
else if(f>=newData.length){updateInfo.removed.push(f);updated=true;}
else if(!(JSON.stringify(self.data[f])===JSON.stringify(newData[f]))){updateInfo.modified.push(f);updated=true;}}
if(updated){self.data=newData;self.dimensionDomains={};calcData(self,data_arr);self.dispatch.call("dataUpdated",self,updateInfo);}}
CINEMA_COMPONENTS.Database.prototype.getSimilar=function(query,threshold){var self=this;var similar=[];this.data.forEach(function(row,index){var dist=0;self.dimensions.forEach(function(d){if(query[d]!==undefined){if(self.isStringDimension(d))
...
emptyValFound=emptyValFound||(data[0][i]===undefined);if(emptyValFound)
return"Empty values may not occur in the header (first line).";var testLength=data[0].length;for(var i in data)
if(data[i].length!=testLength)
return"Each line must have an equal number of comma separated values (columns).";for(var i=0;i<data[0].length;i++){var allEmpty=true;for(var j=0;j<data.length;j++)
allEmpty=allEmpty&&(data[j][i]===undefined);if(allEmpty)
...
this.dimensions.forEach(function(d){self.scales[d].range([self.radius-self.innerMargin,0]);});this.axes.attr('transform',function(d){return self.getAxisTransform(d);}).each(function(d){d3.select(this).select('.axis').call(d3.axisLeft().scale(self.scales[d]));});this.labels.attr('transform',function(d){return self.getAxisTransform(d);});this.path.attr('d',function(d){return self.getPath(self.db.data[d]);});};CINEMA_COMPONENTS.Glyph.prototype.updateData=function(){var self=this;this.dimensions.forEach(function(d){self.scales[d].domain(self.db.dimensionDomains[d]);});this.axes.each(function(d){d3.select(this).select('.axis').call(d3.axisLeft().scale(self.scales[d]));});this.redraw();}
CINEMA_COMPONENTS.Glyph.prototype.setSelected=function(index){this.selected=index;this.redraw();};CINEMA_COMPONENTS.Glyph.prototype.redraw=function(){var self=this;this.path.datum(this.selected).transition(1000).attr('d',function(d){return self.getPath(self.db.data[d]);});}
...
CINEMA_COMPONENTS.ImageSpread.prototype.createModalVTI=function(url){const rootContainer=d3.select('body');const backgroundContainer=d3.select('body').append('div');backgroundContainer.attr('class','modalBackground').on('click',function(){d3.select(this).remove();});const container=backgroundContainer.append('div');container.attr('class','modalViewer').on('click',function(){d3.event.stopPropagation();});var global={};const background=[0,0,0];const fullScreenRenderer=vtk.Rendering.Misc.vtkFullScreenRenderWindow.newInstance({background:background,rootContainer:rootContainer.node(),container:container.node()});const renderer=fullScreenRenderer.getRenderer();const renderWindow=fullScreenRenderer.getRenderWindow();const reader=vtk.IO.Misc.vtkPDBReader.newInstance();const filter=vtk.Filters.General.vtkMoleculeToRepresentation.newInstance();const sphereMapper=vtk.Rendering.Core.vtkSphereMapper.newInstance();const stickMapper=vtk.Rendering.Core.vtkStickMapper.newInstance();const sphereActor=vtk.Rendering.Core.vtkActor.newInstance();const stickActor=vtk.Rendering.Core.vtkActor.newInstance();const vtiReader=vtk.IO.XML.vtkXMLImageDataReader.newInstance();filter.setInputConnection(reader.getOutputPort());filter.setHideElements(['H']);sphereMapper.setInputConnection(filter.getOutputPort(0));sphereMapper.setScaleArray(filter.getSphereScaleArrayName());sphereActor.setMapper(sphereMapper);stickMapper.setInputConnection(filter.getOutputPort(1));stickMapper.setScaleArray('stickScales');stickMapper.setOrientationArray('orientation');stickActor.setMapper(stickMapper);vtk.IO.Core.DataAccessHelper.get().fetchBinary(url,{function(pe){console.log(pe);},}).then((binary)=>{vtiReader.parseAsArrayBuffer(binary);const source=vtiReader.getOutputData(0);const mapper=vtk.Rendering.Core.vtkVolumeMapper.newInstance();const actor=vtk.Rendering.Core.vtkVolume.newInstance();const dataArray=source.getPointData().getScalars()||source.getPointData().getArrays()[0];const dataRange=dataArray.getRange();const lookupTable=vtk.Rendering.Core.vtkColorTransferFunction.newInstance();const piecewiseFunction=vtk.Common.DataModel.vtkPiecewiseFunction.newInstance();actor.setMapper(mapper);mapper.setInputData(source);renderer.addActor(actor);const sampleDistance=0.7*Math.sqrt(source.getSpacing().map((v)=>v*v).reduce((a,b)=>a+b,0));mapper.setSampleDistance(sampleDistance);actor.getProperty().setRGBTransferFunction(0,lookupTable);actor.getProperty().setScalarOpacity(0,piecewiseFunction);actor.getProperty().setInterpolationTypeToLinear();actor.getProperty().setScalarOpacityUnitDistance(0,vtk.Common.DataModel.vtkBoundingBox.getDiagonalLength(source.getBounds())/Math.max(...source.getDimensions()));actor.getProperty().setGradientOpacityMinimumValue(0,0);actor.getProperty().setGradientOpacityMaximumValue(0,(dataRange[1]-dataRange[0])*0.05);actor.getProperty().setShade(true);actor.getProperty().setUseGradientOpacity(0,true);actor.getProperty().setGradientOpacityMinimumOpacity(0,0.0);actor.getProperty().setGradientOpacityMaximumOpacity(0,1.0);actor.getProperty().setAmbient(0.2);actor.getProperty().setDiffuse(0.7);actor.getProperty().setSpecular(0.3);actor.getProperty().setSpecularPower(8.0);const controllerWidget=vtk.Interaction.UI.vtkVolumeController.newInstance({size:[400,150],rescaleColorMap:true,});const isBackgroundDark=background[0]+background[1]+background[2]<1.5;controllerWidget.setContainer(container.node());controllerWidget.setupContent(renderWindow,actor,isBackgroundDark);fullScreenRenderer.setResizeCallback(({width,height})=>{if(width>414){controllerWidget.setSize(400,150);}else{controllerWidget.setSize(width-14,150);}
controllerWidget.render();});renderer.resetCamera();renderWindow.render();global.pipeline={actor,renderer,renderWindow,lookupTable,mapper,source,piecewiseFunction,fullScreenRenderer,};});}
CINEMA_COMPONENTS.ImageSpread.prototype.createModalPDB=function(url){const rootContainer=d3.select('body');const backgroundContainer=d3.select('body').append('div');backgroundContainer.attr('class','modalBackground').on('click',function(){d3.select(this).remove();});const container=backgroundContainer.append('div');container.attr('class','modalViewer').on('click',function(){d3.event.stopPropagation();});var global={};const background=[0,0,0];const fullScreenRenderer=vtk.Rendering.Misc.vtkFullScreenRenderWindow.newInstance({background:background,rootContainer:rootContainer.node(),container:container.node()});const renderer=fullScreenRenderer.getRenderer();const renderWindow=fullScreenRenderer.getRenderWindow();const reader=vtk.IO.Misc.vtkPDBReader.newInstance();const filter=vtk.Filters.General.vtkMoleculeToRepresentation.newInstance();const sphereMapper=vtk.Rendering.Core.vtkSphereMapper.newInstance();const stickMapper=vtk.Rendering.Core.vtkStickMapper.newInstance();const sphereActor=vtk.Rendering.Core.vtkActor.newInstance();const stickActor=vtk.Rendering.Core.vtkActor.newInstance();filter.setInputConnection(reader.getOutputPort());filter.setHideElements(['H']);sphereMapper.setInputConnection(filter.getOutputPort(0));sphereMapper.setScaleArray(filter.getSphereScaleArrayName());sphereActor.setMapper(sphereMapper);stickMapper.setInputConnection(filter.getOutputPort(1));stickMapper.setScaleArray('stickScales');stickMapper.setOrientationArray('orientation');stickActor.setMapper(stickMapper);reader.setUrl(url).then(()=>{renderer.resetCamera();renderWindow.render();});renderer.addActor(sphereActor);renderer.addActor(stickActor);renderer.resetCamera();renderWindow.render();global.reader=reader;global.filter=filter;global.sphereMapper=sphereMapper;global.stickMapper=stickMapper;global.sphereActor=sphereActor;global.stickActor=stickActor;global.renderer=renderer;global.renderWindow=renderWindow;}
CINEMA_COMPONENTS.ImageSpread.prototype.updatePageNav=function(){var self=this;d3.select(this.container).select('.pageNavWrapper').remove();var pageSize=this.pageSizeNode.value;if(this.selection.length>pageSize){var numPages=Math.ceil(this.selection.length/pageSize);if(this.currentPage>numPages){this.currentPage=numPages};d3.select(this.container).append('div').classed('pageNavWrapper',true).append('ul').classed('pageNav',true).selectAll('li').data(getPageButtons(numPages,this.currentPage)).enter().append('li').classed('pageButton',true).attr('mode',function(d){return d.page==self.currentPage?'selected':'default';}).text(function(d){return d.text;}).on('click',function(d){if(d3.select(this).attr('mode')!='selected'){self.currentPage=d.page;if(d.do_rebuild){self.updatePageNav();self.populateResults();}else{d3.select(self.container).select('.pageButton[mode="selected"]').attr('mode','default');d3.select(this).attr('mode','selected');d3.select('.pageReadout').text(self.currentPage+" / "+numPages);self.populateResults();}}});d3.select('.pageNavWrapper').append('div').classed('pageReadout',true).text(this.currentPage+" / "+numPages);}
...
return false;var validFiletypes=['JPG','JPEG','PNG','GIF','VTI','PDB'];type=type.trimLeft().trimRight();var index=validFiletypes.indexOf(type.toUpperCase());return(index>=0);}
...
(this.parentRect.width)+' '+
(this.parentRect.height)).attr('preserveAspectRatio','none').attr('width','100%').attr('height','100%').style('pointer-events','none');this.axes=this.axisContainer.selectAll('.axisGroup').data(this.dimensions).enter().append('g').classed('axisGroup',true).attr('dimension',function(d){return d;}).attr('transform',function(d){return"translate("+self.x(d)+")";}).call(this.drag)
...
self.addNaNExtensionToAxis(this);});var labels=this.axes.append('g').classed('axisLabel',true).style('pointer-events','initial')
labels.append('text').style('text-anchor','middle').attr('y',-9).text(function(d){return d;});labels.insert('rect',':first-child').data(function(){var boxes=[];labels.selectAll('text').each(function(){boxes.push(this.getBBox());});return boxes;}).attr('x',function(d){return d.x+3;}).attr('y',function(d){return d.y;}).attr('width',function(d){return d.width-6;}).attr('height',function(d){return d.height;});this.axes.append('g').classed('brush',true).each(function(){d3.select(this).call(self.brush);});};CINEMA_COMPONENTS.Pcoord.prototype=Object.create(CINEMA_COMPONENTS.Component.prototype);CINEMA_COMPONENTS.Pcoord.prototype.constructor=CINEMA_COMPONENTS.Pcoord;CINEMA_COMPONENTS.Pcoord.prototype.addNaNExtensionToAxis=function(node){d3.select(node).append('path').attr('class','NaNExtension').attr('d',"M0.5,"+String(this.internalHeight-this.NaNMargin+0.5)+"V"+String(this.internalHeight-0.5));var NaNTick=d3.select(node).append('g').attr('class','NaNExtensionTick').attr('transform',"translate(0,"+String(this.internalHeight-0.5)+")");NaNTick.append('line').attr('x2','-6');NaNTick.append('text').attr('x','-9').attr('dy','0.32em').text('NaN');}
...
CINEMA_COMPONENTS.Pcoord.prototype.updateData=function(){var self=this;this.dimensions.forEach(function(d){self.y[d].domain(self.db.dimensionDomains[d]);});this.axes.each(function(d){d3.select(this).select('.axis').call(d3.axisLeft().scale(self.y[d]));});this.updateSelection(true);}
...
break;}}
if(selected)
newSelection.push(i);});if(!arraysEqual(this.selection,newSelection)||force){this.selection=newSelection;this.dispatch.call("selectionchange",this,this.selection.slice());this.redrawSelectedPaths();}}
...
CINEMA_COMPONENTS.Pcoord.prototype.filterSelection=function(filter){var self=this;this.dimensions.forEach(function(d){var f=filter?filter[d]:null
if(f&&Array.isArray(f)&&f.length==2&&!isNaN(f[0])&&!isNaN(f[1])){var range=[Math.max(self.y[d](f[1]),0),Math.min(self.y[d](f[0]),self.internalHeight)]
self.axisContainer.select('.axisGroup[dimension='+d+']').select('g.brush').call(self.brush.move,function(){return range;})}});this.axisBrush();}
...
order.push(d);});this.x.domain(order);self.dimensions.sort(function(a,b){return self.getXPosition(a)-self.getXPosition(b);});this.axes.attr('transform',function(d){return"translate("+self.getXPosition(d)+")";});self.axes.sort(function(a,b){return self.x(a)-self.x(b);});this.redrawPaths();}
